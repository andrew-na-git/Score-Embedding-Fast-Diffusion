...
zero false positives on our control sequence. We instantiate the pipeline for
...
M-matrix stability; sequential importance sampling; video inpainting.
...
\begin{enumerate}
  ...
  \item A \textbf{KL-triggered sequential importance sampler} that reuses the
...
literature~\cite{doucet-2001-smc,kong-1994-ess}. Motion fields use
...
\begin{itemize}
  ...
  \item \textbf{Cross-clip warm starts do not help.} Measured at $1.12\times$ and
        removed: upwinding already converges in $4$--$5$ iterations from cold,
        leaving nothing to save. Recorded so it is not reintroduced.
...
is validated and reproducible. We instantiate it for per-instance video
